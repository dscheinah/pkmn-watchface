...
bool enemy_evolution(State* state) {
  if (state->event & EVENT_MORPH) {
    if (state->enemy->type == RESOURCE_ID_132) {
      helper_evolve(state, rand() % 6 + ENEMY_OFFSET + 24, state->enemy->level_multiplier, false);
    } else {
  ...
  if (state->event & EVENT_EVO) {
    switch (state->enemy->type) {
      case RESOURCE_ID_133:
        if (state->enemy->hours_alive > 4 || (state->enemy->hours_alive > 3 && rand() % 2 == 0)) {
          helper_evolve(state, RESOURCE_ID_196, 2, false);
        } else {
          helper_evolve(state, state->enemy->type + 1 + rand() % 2, 2, false);
        }
        return true;
      case RESOURCE_ID_86:
        ...
        return true;
      default:
        if (state->enemy->type >= ENEMY_OFFSET && state->enemy->type <= ENEMY_OFFSET + 5) {
